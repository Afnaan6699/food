    ...
    <div className="px-6 lg:px-12 py-10 pb-24">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-5 mb-8 animate-fade-up">
          <KPI icon={Utensils} label="Total Meals Saved" value="128,540" delta="+12.4%" />
          <KPI icon={Trash2} label="Food Waste Reduced" value="42,180 kg" delta="+8.7%" />
          <KPI icon={Cloud} label="CO₂ Prevented" value="86.2 tons" delta="+15.2%" />
          <KPI icon={Truck} label="Active Deliveries" value="1,284" delta="-3.1%" positive={true} />
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-6 animate-fade-up" style={{ animationDelay: '150ms' }}>
          ...
          <div className="glass rounded-3xl p-6 shadow-elegant border border-white/5 flex flex-col">
            ...
            <div className="h-64 flex-1 mt-4">
              <ResponsiveContainer width="100%" height="100%">
                <PieChart>
                  <Pie data={types} dataKey="value" cx="50%" cy="50%" innerRadius={60} outerRadius={90} paddingAngle={2} stroke="none">
                    {types.map((t, i) => <Cell key={i} fill={t.color} />)}
                  </Pie>
